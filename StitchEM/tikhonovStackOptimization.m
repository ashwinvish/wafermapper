function sections = tikhonovStackOptimization(params, sections)
...
for s = 1:length(sections)
    for i = 1:length(sections(s).tiles)
        for j = i:length(sections(s).tiles)
            if any(j == sections(s).tiles(i).neighbors)
                pts_i = [sections(s).tiles(i).pts{j} ones(size(sections(s).tiles(i).pts{j}, 1), 1)];
                pts_j = [sections(s).tiles(j).pts{i} ones(size(sections(s).tiles(j).pts{i}, 1), 1)];
                seam = zeros(size(pts_i, 1), length(sections(s).tiles) * 3);
