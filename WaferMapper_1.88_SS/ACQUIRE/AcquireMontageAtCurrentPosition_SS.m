function[logBook] =  AcquireMontageAtCurrentPosition_SS(WaferName, LabelStr,logBook)
...
% figure(33);
% for x=1:NumRowTiles*NumColTiles
%     plot([(listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)-(FOV_microns/2) (listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)+(FOV_microns/2)],[(listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)+(FOV_microns/2) (listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)+(FOV_microns/2)],'b-');
%     hold on
%     plot([(listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)+(FOV_microns/2) (listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)+(FOV_microns/2)],[(listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)+(FOV_microns/2) (listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)-(FOV_microns/2)],'b-');
%     plot([(listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)+(FOV_microns/2) (listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)-(FOV_microns/2)],[(listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)-(FOV_microns/2) (listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)-(FOV_microns/2)],'b-');
%     plot([(listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)-(FOV_microns/2) (listStageX_Meters(x)*1000000-listStageX_Meters(1)*1000000)-(FOV_microns/2)],[(listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)-(FOV_microns/2) (listStageY_Meters(x)*1000000-listStageY_Meters(1)*1000000)+(FOV_microns/2)],'b-');
% end
% eval(sprintf('print(gcf,''-r100'',''-dpng'',''-painters'',''%s/MontageOutline.png'');',MontageDirName));
% close(gcf)
% clear x
...
if GuiGlobalsStruct.MontageParameters.IsSingle_AF_ForWholeMontage
    ...
    %added by AV to begin focus where previous step had left off
...
if GuiGlobalsStruct.MontageParameters.IsSingle_AFASAF_ForWholeMontage
    ...
    autoBrightCon_SSAF;
    NewMagForFocus = 1000;
    StartingMagForAFn = 1000;
    GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('AP_Contrast',NewContrastForFocus);
    GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('AP_Brightness',NewBrightnessForFocus);
    GuiGlobalsStruct.MyCZEMAPIClass.MoveStage(StageX_Meters,StageY_Meters,stage_z,stage_t,stage_r,stage_m);
    smartTileFocus(StartingMagForAFn, IsPerformAutoStig, StartingMagForAS, focOptions);
    %old
    %smartTileFocus(StartingMagForAF, IsPerformAutoStig, StartingMagForAS, focOptions);
    %----
...
if GuiGlobalsStruct.MontageParameters.IsAFOnEveryTileMAPFoSt
    ...
    %performing MAPFost three times successively.
    StartingWDForAF = GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_WD');
    smartTileFocusMAPFoSt(StartingWDForAF, StartingMagForAF, IsPerformAutoStig, StartingMagForAS, focOptions);
    StartingWDForAF = GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_WD');
    smartTileFocusMAPFoSt(StartingWDForAF, StartingMagForAF, IsPerformAutoStig, StartingMagForAS, focOptions);
    StartingWDForAF = GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_WD');
    smartTileFocusMAPFoSt(StartingWDForAF, StartingMagForAF, IsPerformAutoStig, StartingMagForAS, focOptions);
    
    GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('DP_BEAM_BLANKED',0)
...
% %% Do AF+AS+AF on desired Tile - Added by AV 04/04/2017
%         % Go to desired tile
%         GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('DP_EXT_SCAN_CONTROL',0);
%         GuiGlobalsStruct.MyCZEMAPIClass.MoveStage(listStageX_Meters(13),listStageY_Meters(13),stage_z,stage_t,stage_r,stage_m); % hard coded tile
%         while(strcmp(GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeString('DP_STAGE_IS'),'Busy'))
%             pause(.02)
%         end
%         wmBackLash
...
for tileGroup = 1:    length(groupTiles)
    ...
    for t= 1:length(tileList)
        ...
        if max([xDif yDif])>missedThresh
            ...
            fprintf('Missed tile target. Move again. (Focus)\n');
        ...
        GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('AP_SCANROTATION',theta_Degrees);
        autoBrightCon_SS2
        AutoBCSave(tL,1)=newBright;
        AutoBCSave(tL,2)=newCon;
    ...
    fprintf('Overall on AutoBC: %g m %g s\n',to2m,round(to2s));
    fprintf('\n');
    %%new end
    ...
    for t= 1:length(tileList)
        ...
        for tileAttempt = 1:numTileAttempts  %number of times to try taking good image
            ...
            if max([xDif yDif])>missedThresh
                ...
                missed.section = LabelStr;
                missed.tile = allTiles(tL,:);
                fprintf('Missed tile target. Move again. (First Try)\n');
            ...
            if max([xDif yDif])>missedThresh
                ...
                missed.section = LabelStr;
                missed.tile = allTiles(tL,:);
                fprintf('Missed tile target. Move again. (Second Try)\n');
            ...
            AutoBCSave=sort(AutoBCSave);
            GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('AP_Contrast',median(AutoBCSave(:,2))-(AutoBCSave(end-2,2)-AutoBCSave(3,2)));
            GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('AP_Brightness',median(AutoBCSave(:,1))+(AutoBCSave(3,1)-AutoBCSave(end-2,1)));
            GuiGlobalsStruct.MyCZEMAPIClass.Set_PassedTypeSingle('AP_SCANROTATION',theta_Degrees);
            ...
            if IsDisplay
                ...
                figure(StitchFigNum);
                clf;
                title(MontageDirName,'Interpreter','none');
