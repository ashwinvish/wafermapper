...
if GuiGlobalsStruct.MontageParameters.IsSingle_AFASAF_ForWholeMontage
    ...
    %---- changed 20180702
     ForAutoBrightnessContrastX = mean(listStageX_Meters);
     ForAutoBrightnessContrastY = mean(listStageY_Meters);
     GuiGlobalsStruct.MyCZEMAPIClass.MoveStage(ForAutoBrightnessContrastX,ForAutoBrightnessContrastY,stage_z,stage_t,stage_r,stage_m);
     
     AutoBCFOV_microns = GuiGlobalsStruct.MontageParameters.TileFOV_microns*(max(allTiles(:)*1.5));
     AutoBCImageWidthInPixels = 1024;
     AutoBCImageHeightInPixels = 1024;
     AutoBCDwellTimeInMicroseconds = .5;
     AutoBCIsDoAutoRetakeIfNeeded = false;
     AutoBCIsMagOverride = false;
     AutoBCMagForOverride = -1;
     AutoBCWaferNameStr = WaferName;
     AutoBCLabelStr = LabelStr; 
     
     AutoBCImageFileNameStr = sprintf('%s\\AutoBCImage_%s.tif', MontageDirName, WaferName);
     Fibics_AcquireImage(AutoBCImageWidthInPixels, AutoBCImageHeightInPixels, AutoBCDwellTimeInMicroseconds, AutoBCImageFileNameStr,...
         AutoBCFOV_microns, AutoBCIsDoAutoRetakeIfNeeded, AutoBCIsMagOverride, AutoBCMagForOverride,  AutoBCWaferNameStr, AutoBCLabelStr);%
     
     BCImage = imread(AutoBCImageFileNameStr);
     [BCImageHistCounts,BCImageHistLogs] = imhist(BCImage);
   
     NewContrastForFocus = 45;
    NewBrightnessForFocus = 52;
    NewMagForFocus = 2320;
    StartingMagForAFn = 2320;
