...
% Last Modified by GUIDE v2.5 05-Jul-2018 10:29:04
...
if dirname == 0
    ...
    %non UTSL directory.
    UTSLDefaultsFileNameStr = [dirname filesep 'UTSLDefaults.mat'];
...
if ~exist(GuiGlobalsStruct.UTSLDirectory, 'dir')
    ...
    GuiGlobalsStruct.LowResFiducialsDirectory = [GuiGlobalsStruct.WaferDirectory filesep 'LowResFiducialsDirectory'];
    GuiGlobalsStruct.HighResFiducialsDirectory = [GuiGlobalsStruct.WaferDirectory filesep 'HighResFiducialsDirectory'];
...
%
    ...
    if exist(GuiGlobalsStruct.FullWaferTileImagesDirectory,'dir')
        ...
        if exist(GuiGlobalsStruct.FullWaferTileImagesDirectory,'dir')
            CoarseSectionListFileNameStr = [GuiGlobalsStruct.FullWaferTileImagesDirectory filesep 'CoarseSectionList.mat'];
    ...
    if exist(GuiGlobalsStruct.PixelToStageCalibrationDirectory,'dir')
        ...
        if exist(MyTempFileName, 'file')
            CalibrationFileNameStr = [GuiGlobalsStruct.PixelToStageCalibrationDirectory filesep 'CalibrationFile.mat'];
    ...
    if exist(GuiGlobalsStruct.SectionOverviewTemplateDirectory,'dir')
       SectionOverviewTemplateFileNameStr = [GuiGlobalsStruct.SectionOverviewTemplateDirectory filesep 'SectionOverviewTemplate.tif'];
...
if exist(GuiGlobalsStruct.ExampleSectionImageDirectory, 'dir')
    MyStr = [GuiGlobalsStruct.ExampleSectionImageDirectory ' already exists. If you are sure you want to acquire a new example image for this wafer then manually delete the directory.'];
    uiwait(msgbox(MyStr,'modal'));
    return;
else
...
WaferDirName = [GuiGlobalsStruct.UTSLDirectory filesep WaferName];
if ~exist(WaferDirName, 'dir')
    WaferDirName = GuiGlobalsStruct.WaferDirectory;
end


ImageFileNameStr = [WaferDirName filesep 'SectionOverviewsAlignedWithTemplateDirectory' filesep 'SectionOverviewAligned_' LabelStr '.tif'];
...
%Check if this directory exists and change to current wafer name if not
WaferDirName = [GuiGlobalsStruct.UTSLDirectory filesep '%s' WaferName];
if ~exist(WaferDirName, 'dir')
    WaferDirName = GuiGlobalsStruct.WaferDirectory;
end

ImageFileNameStr = [WaferDirName filesep 'SectionOverviewsAlignedWithTemplateDirectory' filesep ...
    'SectionOverviewAligned_' LabelStr '.tif'];
if ~exist(ImageFileNameStr, 'file')
...
GuiGlobalsStruct.ReimageLowResFiducialsDirectory = sprintf(['%s' filesep 'ReimageLowResFiducialsDirectory'],GuiGlobalsStruct.WaferDirectory);
...
if IsPauseForFeedback
    ...
    GuiGlobalsStruct.ReimageWithCorrectionLowResFiducialsDirectory = ['%s' GuiGlobalsStruct.WaferDirectory filesep 'ReimageWithCorrectionLowResFiducialsDirectory'];
...
try
    doSURF = GuiGlobalsStruct.SectionOverviewProcessingParameters.SURF;
catch err
    doSURF = 0;
    GuiGlobalsStruct.SectionOverviewProcessingParameters.SURF = 0;
end
...
CroppedImageFileNameStr = [GuiGlobalsStruct.SectionOverviewTemplateDirectory filesep 'SectionOverviewTemplateCropped.tif'];
if exist(CroppedImageFileNameStr, 'file')
...
% handles    structure with handles and user data (see GUIDATA)
global GuiGlobalsStruct;
if isfield(GuiGlobalsStruct,'SurfOptions')
[SurfOptions,RanSacOptions,MetaAlignmentOptions]=...
    ProcessOverviewsRobustRigidParameters2(GuiGlobalsStruct.SurfOptions,...
    GuiGlobalsStruct.RanSacOptions,GuiGlobalsStruct.MetaAlignmentOptions);
else
    [SurfOptions,RanSacOptions,MetaAlignmentOptions]=ProcessOverviewsRobustRigidParameters2();
end
GuiGlobalsStruct.SurfOptions=SurfOptions;
GuiGlobalsStruct.RanSacOptions=RanSacOptions;
GuiGlobalsStruct.MetaAlignmentOptions=MetaAlignmentOptions;
% --------------------------------------------------------------------
...
ValStr = get(handles.SectionLabel_EditBox,'String');
DataFileNameStr = [GuiGlobalsStruct.ManuallyCorrectedStagePositionsDirectory filesep 'CorrectedStagePosition_' ValStr '.mat'];
save(DataFileNameStr,'Info');
disp(['Saved ' DataFileNameStr]);
...
% --------------------------------------------------------------------
function AutoBCforSectionOverviews_MenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to AutoBCforSectionOverviews_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global GuiGlobalsStruct;

AcquireOverviewImageswBC;


% --------------------------------------------------------------------
function AcquireMontageStackMainwBC_MneuItem_Callback(hObject, eventdata, handles)
% hObject    handle to AcquireMontageStackMainwBC_MneuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
AcquireMontageStack_Main_SS(handles);
