...
for i = 1:length(GuiGlobalsStruct.CoarseSectionList)
    ...
    if(~GuiGlobalsStruct.CoarseSectionList(i).IsDeleted)
        ImageFileNameStr = sprintf('%s\\SectionOverviewBC_%s.tif',GuiGlobalsStruct.SectionOverviewsDirectory,GuiGlobalsStruct.CoarseSectionList(i).Label);
...
%             %% Delay to read and display image 
%               IsReadOK = false;
%             
%             while ~IsReadOK
%                 IsReadOK = true;
%                 try
%                     MyImage = dir(ImageFileNameStr);
%                 catch MyException
%                     IsReadOK = false;
%                     pause(.1);
%                 end
